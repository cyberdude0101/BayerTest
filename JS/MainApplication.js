...
gauge.add($("NWSGaugeDiv"), {width:400, height:30, name: 'NWSGauge', limit: true, gradient: true, scale: 10, colors:['#E31A1C','#FC4E2A','#FEB24C','#D9F0A3','#C8F60F','#83F60F'], values:[35,10,5,10,10,30,5]});
gauge.add($("MesoGaugeDiv"), {width:400, height:30, name: 'MesoGauge', limit: true, gradient: true, scale: 10, colors:['#E31A1C','#FC4E2A','#E67451','#ADDD8E','#F7FCB9','#C8F60F','#83F60F'], values:[20,20,20,2,5,18,14]});
gauge.add($("DD60GaugeDiv"), {width:400, height:30, name: 'DD60Gauge', limit: true, gradient: true, scale: 10, colors:['#C11B17','#F66F0F','#F6BD0F','#C8F60F','#83F60F'], values:[10,10,10,10,10,10]});
...
control.events.register("featureunselected", this, function(e) {
 ...
 info = new OpenLayers.Control.WMSGetFeatureInfo({
       ...
       DD605DayV = e.features[0].attributes.DD605Day;
       ...
       CountyName = e.features[0].attributes.CountyName;
       DD605DayCondV = e.features[0].attributes.DD60Cond;
//-----------------Gauje Update---------------------------------------------------
   ...
   if (NWSMinTemp <= 35 )  
              ...
              $j("#NWSCondVLabel").text("is less than 50°F threshold");
              }
              ...
              else if (NWSMinTemp > 45 && NWSMinTemp <= 49.9 )  
              {
              ...
              else if (NWSMinTemp > 49.9 && NWSMinTemp < 50.1 )  
              {
               gauge.modify($('NWSGauge'), {values:[0.35,0.10,0.10,1]});
                document.getElementById('NWSMinTempLabel').style.color = "#FF0000";
                document.getElementById('NWSMinTempLabel').style.fontWeight= "bolder";
                $j("#NWSCondVLabel").text("is at 50°F threshold");
              }

              else if (NWSMinTemp >= 50.1  && NWSMinTemp <60 )  
              {
               gauge.modify($('NWSGauge'), {values:[0.35,0.10,0.05,0.10,1]});
                document.getElementById('NWSMinTempLabel').style.color = "#003300";
                document.getElementById('NWSMinTempLabel').style.fontWeight= "bolder";
                 $j("#NWSCondVLabel").text("is greater than 50°F threshold");
              }
             ...
             if (MesoTemp > 1 && MesoTemp <= 20 )  
              ...
              $j("#MesoCondVLabel").text("is less than 68°F threshold");
               ...
               else if (MesoTemp == 1 )  
              {
               ...
               document.getElementById('MesoTempLabel').style.color = "#FF0000";
               document.getElementById('MesoTempLabel').style.fontWeight= "bolder";
                $j("#MesoCondVLabel").text("is not available");
              ...
              else if (MesoTemp >= 68 )  
              {
               ...
               document.getElementById('MesoTempLabel').style.fontWeight= "bolder";
               $j("#MesoCondVLabel").text("is greater than 68°F threshold");
...
if (FinalRank == "Planting Not Recommended")
          	{
          		$j("#FinalRankLabel").text("Planting Not Recommended");
          		$j('#FinalRankLabel').css('color','#FF3300');
          		$j('#FinalRankLabel').css('fontWeight','bolder');

				
          		
          	}
            else if (FinalRank == "No automated forecast available due to missing soil sensor. Enter 2 in Soil Temperaure to obtain forecast")
          	{
          		$j("#FinalRankLabel").text("No automated forecast available due to missing soil sensor. Enter 2 in Soil Temperaure to obtain forecast.");
          		$j('#FinalRankLabel').css('color','#FF3300');
          		$j('#FinalRankLabel').css('fontWeight','bolder');

          		
          	}
          	 else if (FinalRank == "Soil temp at planting depth is marginal for planting.  Use caution.")
          	{
          		$j("#FinalRankLabel").text("Soil temp at planting depth is marginal for planting.  Use caution !!");
          		$j('#FinalRankLabel').css('color','#33CC33');
          		$j('#FinalRankLabel').css('fontWeight','bolder');

          		 
          	}
             else if (FinalRank == "Marginal" || FinalRank == "Good" || FinalRank =="Very Good")	
             {	
				 $j("#FinalRankLabel").text("Planting Condtions are" + FinalRank);
				 $j('#FinalRankLabel').css('color','#33CC33');
          		$j('#FinalRankLabel').css('fontWeight','bolder');
     ...
     $j("#DispDateLabel").text(DispDate);
      ...
      $j("#CountyNameLabel").text(CountyName);

     $j("#DD605DayCondVLabel").text(DD605DayCondV);


    //     $j.mobile.changePage("#popup");
       DisplayAlert('attributePage',250,50);
$j.mobile.hidePageLoadingMsg();  
        }
}
	    );
    ...
    function DisplayAlert(id,left,top) {
		...
		//	document.getElementById(id).style.top=top+'px';
			...
			function DisplayLayerAlert(id,right,top) {
		//	document.getElementById(id).style.left=left+'px';
		//	document.getElementById(id).style.top=top+'px';
			document.getElementById(id).style.display='list-item';
       document.getElementById('attributePage').style.display='none'
			}
    //------------------USER SOIL TEMP----------------------------------------------------
function Calculate() {
...
var SoilTempRank;
 if (SoilTemp >= 67.5 && SoilTemp <= 100)
        ...
        else if(SoilTemp < 67.4 && SoilTemp >= 65)
    {
     if (AirCondition != "Planting NOT recommended due to min air temp forecast for next 5 days is 50 F or below" && DD605DayV > 15)
     {
       $j("#FinalRankLabel").text("Soil temp at planting depth is marginal for planting. Use caution.");
     }
      else		
	...
	else if (SoilTemp <=65 && SoilTemp>= 32)
	{
